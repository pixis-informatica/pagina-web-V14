...
if ($isWhatsApp || strpos($userAgent, 'facebookexternalhit') !== false) {
    // Forzamos detección de WhatsApp privado para servir imagen directa
    $isWhatsApp = true; 
}
...
/**
 * Convierte la ruta de imagen a una URL absoluta válida
 */
...
if ($bannerId) {
    ...
    if ($siteData) {
        ...
        $fallbackBanners = ['kitryzen' => ['t' => 'Kits de Actualización Ryzen'], 'pccombo' => ['t' => 'PC Gamers y Combos'], 'monitor' => ['t' => 'Monitores Raptor'], 'rtx5050' => ['t' => 'Tarjetas Gráficas RTX 5050'], 'rtx5060' => ['t' => 'Tarjetas Gráficas RTX 5060'], 'notebooks' => ['t' => 'Notebooks y Mini PCs'], 'prolongadores' => ['t' => 'Prolongadores Kelyx'], 'ssd-hiksemi' => ['t' => 'SSDs Hiksemi'], 'refrigeracion-raptor' => ['t' => 'Refrigeración Raptor'], 'gabinetes-raptor' => ['t' => 'Gabinetes Raptor'], 'perifericos-raptor' => ['t' => 'Periféricos Raptor']];
        if (isset($siteBanners[$bannerId])) { $theBanner = $siteBanners[$bannerId]; } elseif (isset($fallbackBanners[$bannerId])) { $theBanner = $fallbackBanners[$bannerId]; }
        if ($theBanner) {
    ...
    if ($prodsData) {
        ...
        if (is_array($products)) { foreach ($products as $p) { $currentSlug = makeSlug($p['title'] ?? 'producto'); if ($currentSlug === $slug) { $theProduct = $p; break; } } }
    }
}
...
if ($theProduct) {
    ...
    $description = "Aprovechá las mejores ofertas en " . $bannerTitle . ". Envíos a todo el país.";
    ...
    $title = "Categoría " . $categoryName . " - Pixis Informatica";
    $description = "Encontrá los mejores productos de " . $categoryName . " en Pixis Informática.";
    $imgSource = $theCategory['img'] ?? ($theCategory['customIcon'] ?? 'img/logo_pixis.png');
    if (isset($CATEGORIAS_SEO[$catLower])) {
        if (!empty($CATEGORIAS_SEO[$catLower]['titulo'])) $title = $CATEGORIAS_SEO[$catLower]['titulo'];
        if (!empty($CATEGORIAS_SEO[$catLower]['descripcion'])) $description = $CATEGORIAS_SEO[$catLower]['descripcion'];
        if (empty($theCategory['customIcon']) && !empty($CATEGORIAS_SEO[$catLower]['imagen'])) $imgSource = $CATEGORIAS_SEO[$catLower]['imagen'];
    }
